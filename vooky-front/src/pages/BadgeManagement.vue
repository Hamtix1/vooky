...
function getBadgeImageUrl(image: string): string {
  if (!image) return '';
  // Si la imagen ya es URL completa, el helper la retorna tal cual
  // Si la imagen empieza por /storage, usar tal cual
  // Si empieza por storage, agregar /
  // Si empieza por /badges o badges, agregar /storage delante
  let path = image;
